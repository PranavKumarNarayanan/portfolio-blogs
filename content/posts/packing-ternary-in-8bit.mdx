---
title: "How to pack ternary numbers in 8-bit bytes"
date: "2026-07-21"
summary: "with efficient SIMD-friendly unpacking"
---
...
I've been recently nerd-sniped into trying to pack the ternary weights of [BitNet b1.58](https://arxiv.org/abs/2402.17764) into something close to that theoretical ideal of `log(3) / log(2)` bits per ternary digit
...
| trits | 3<sup>trits</sup> | bits | 2<sup>bits</sup> | bits per trit |
| ----: | ----------------: | ---: | ---------------: | :------------ |
|     1 |                 3 |    2 |                4 | 2             |
|     2 |                 9 |    4 |               16 | 2             |
|     3 |                27 |    5 |               32 | 1.666...      |
...
While repeated remainder and divisions can be used to extract the digits of a number, the problem with divisions and modulo is that they are not usually supported on integers in SIMD (Single Instruction Multiple Data) programming.
...
move 4.5cm
E: box "0x86." fit
arrow -> from A to B chop "same number" above
arrow <- from C to B chop "divide by 243" aligned below
arrow -> from C.se to D.nw chop "same number, round up" aligned below
arrow <- from E to D.sw chop "multiply by 256" aligned below
```
Tada!
...
def unpack_trits(packed: bytes) -> list[int]:
    ...
    return trits
```
To convince myself that this works, I wrote a C program checking that this really is lossless (yeah, I'm an idiot):
